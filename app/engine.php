...
$currencies=array();
foreach($currency_settings as $letters=>$enabled){$currencies[''.$letters.'']=$enabled;}
...
header('location: /signup/');exit;}else{$_SESSION['ref']=NULL;}
}

if($_SESSION['ref']!=NULL){
$query = $db->prepare("SELECT isactive FROM users WHERE id = ?");
$query->execute(array($_SESSION['ref']));
if($query->rowCount()==0 || $query->fetch(\PDO::FETCH_ASSOC)['isactive']=='0'){$_SESSION['ref']=NULL;}
}
...
header('location: /signup/');exit;}else{$_SESSION['link']=NULL;}
}

if($_SESSION['link']!=NULL){
$query = $db->prepare("SELECT id FROM referral_links WHERE link = ? AND product_id = ?");
$query->execute(array($_SESSION['link'],$product['id']));
if($query->rowCount()==0){$_SESSION['link']=NULL;}else{$_SESSION['ref']=NULL;}
}
