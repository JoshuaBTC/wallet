...
if (time() > ($query->fetch(\PDO::FETCH_ASSOC)['time'] + 300)) {
    ...
    if (count($prices) != count($currencies)) {
        foreach ($currencies as $currency => $enabled) {
            ...
            if ($request !== false && isset(json_decode($request, true)['ticker']['price'])) {
                $prices['' . $currency . ''] = json_decode($request, true)['ticker']['price'];
            }
        }
    }
    ...
    if (count($prices) != count($currencies)) {
        foreach ($currencies as $currency) {
            if (!isset($prices['' . $currency . ''])) {
                ...
                if ($request !== false && isset(json_decode($request, true)['USD'])) {
                    $prices['' . $currency . ''] = json_decode($request, true)['USD'];
                }
            }
        }
    }
    ...
    if (is_array($prices) && count($prices) == count($currencies)) {
        ...
        foreach ($prices as $paramKey => $paramValue) {
            ...
            foreach ($currencies as $currency) {
                if (!isset($prices['' . $currency . ''])) {
                    if ($currency == 'ETH') {
                        ...
                        if ($request !== false && isset(json_decode($request, true)['ETH'])) {
                            $prices['' . $currency . ''] = json_decode($request, true)['ETH'];
                        }
                    }
                }
            }
...
function convertFromSatoshi($value)
{
    return rtrim(rtrim(bcdiv($value, "1000000000000000000", 18), 0), '.');
}

if (time() > ($product['ethereum_timestamp'] + 300)) {
...
if (!isset($_SESSION['user_language'])) {
    $i = 0;
    foreach ($country_codes as $language_title) {
        if (in_array($user['country_code'], $country_codes[array_keys($country_codes)[$i]])) {
            $_SESSION['user_language'] = array_keys($country_codes)[$i];
        } else {
...
#Определение по ip невозможно, проверяем язык браузера
    if ($_SESSION['user_language'] == 'en') {
        ...
        if (is_array($localePreferences) && count($localePreferences) > 0) {
            ...
            $i = 0;
            foreach ($locale_codes as $language_title) {
                if (in_array($browserLocale, $locale_codes[array_keys($locale_codes)[$i]])) {
                    $_SESSION['user_language'] = array_keys($locale_codes)[$i];
                }
                $i++;
            }
            unset($i);
...
if (isset($_GET['lng']) && in_array($_GET['lng'], $available_languages)) {
    $_SESSION['user_language'] = $_GET['lng'];
    $location_add = '';
    if (in_array(rtrim(str_replace($product['url'], '', $_SERVER['HTTP_REFERER']), '/'), array('buy', 'profile', 'signup', 'signin', 'reset', 'support', 'admin', 'two_factor'))) {
        $location_add = rtrim(str_replace($product['url'], '', $_SERVER['HTTP_REFERER']), '/') . '/';
    }
    header('location: /' . $location_add . '');
    exit();
}
...
if ($logged_in['flag']) {
    ...
    if ($user['email'] == '' && $user['oauth_email'] != '') {
        $user['email'] = $user['oauth_email'];
    }

#Кошельки пользователя
    if ($user['ethereum_address_provided'] == '0') {
        $user['ethereum_address'] = '';
    }
    if ($user['SBD_address_provided'] == '0') {
        $user['SBD_address'] = '';
    }
    if ($user['GOLOS_address_provided'] == '0') {
        $user['GOLOS_address'] = '';
    }
    if ($user['BTS_address_provided'] == '0') {
        $user['BTS_address'] = '';
    }
...
#Переход по реферальной ссылке пользователя
    ...
    if (isset($_GET['ref']) && is_numeric($_GET['ref']) && $_GET['ref'] > 0) {
        ...
        if ($query->rowCount() > 0 && $query->fetch(\PDO::FETCH_ASSOC)['isactive'] == '1') {
            ...
            $query = $db->prepare("UPDATE users SET referral_visits = referral_visits + ? WHERE id = ? AND product_id = ?");
            $query->execute(array('1', $_SESSION['ref'], $product['id']));
    ...
    if ($_SESSION['ref'] != NULL) {
        $query = $db->prepare("SELECT isactive FROM users WHERE id = ?");
        $query->execute(array($_SESSION['ref']));
        if ($query->rowCount() == 0 || $query->fetch(\PDO::FETCH_ASSOC)['isactive'] == '0') {
            $_SESSION['ref'] = NULL;
        }
    }

#Переход по созданной реферальной ссылке
    if (!isset($_SESSION['link'])) {
        $_SESSION['link'] = NULL;
    }
    if (isset($_GET['link']) && strlen($_GET['link']) == 8) {
        $query = $db->prepare("SELECT id FROM referral_links WHERE link = ? AND product_id = ?");
        $query->execute(array($_GET['link'], $product['id']));
        if ($query->rowCount() > 0) {
            $_SESSION['link'] = $_GET['link'];

            $query = $db->prepare("UPDATE referral_links SET referral_visits = referral_visits + ? WHERE link = ? AND product_id = ?");
            $query->execute(array('1', $_SESSION['link'], $product['id']));
    ...
    if ($_SESSION['link'] != NULL) {
        $query = $db->prepare("SELECT id FROM referral_links WHERE link = ? AND product_id = ?");
        $query->execute(array($_SESSION['link'], $product['id']));
        if ($query->rowCount() == 0) {
            $_SESSION['link'] = NULL;
        } else {
            $_SESSION['ref'] = NULL;
...
function answer_json($status, $message = '', $array = array())
{
    header('Content-type:application/json;charset=utf-8');
    $encode = array_merge(array('status' => $status, 'message' => $message), $array);
...
function sendmail($to, $email_images, $subject, $body, $altbody)
{
    ...
    try {
        ...
        foreach ($email_images as $image) {
            $mail->AddEmbeddedImage(__DIR__ . '/assets/products/' . $product['id'] . '/mail/' . $image . '.png', $image, $image);
        }
