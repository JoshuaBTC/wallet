...
if($option!='list'){answer_json('error',$lng['error_field_required']);}

$query=$db->prepare("SELECT * FROM users WHERE product_id=?");
$query->execute(array($product['id']));
if($query->rowCount()==0){answer_json('no_users',$lng['error_user_not_found']);}
while($usr_row=$query->fetch(\PDO::FETCH_ASSOC)){print_r($usr_row);}
...
if($token_decimals=='' || $token_decimals<0 || $token_decimals>30){answer_json('token_decimals',$lng['error_field_required']);}
...
if($other_raised==''){answer_json('other_raised',$lng['error_field_required']);}
...
if(!$query->execute(array_values(array($token_price,$token_price_eth,$other_raised,$bonus_percent,$referral_percent,$signup_tokens,$support_email,$email_activation,$ethereum_field_required,$SBD_wallet,$GOLOS_wallet,$BTS_wallet,$product['id'])))){answer_json('error',$lng['error_system']);}else{answer_json('success',$lng['admin_success']);}
...
if($referral_link==''){answer_json('referral_link',$lng['error_field_required']);}

if(stristr($referral_link,$product['url_main'].'?ref=')){$type='user';$link=str_replace($product['url_main'].'?ref=','',$referral_link);}
if(stristr($referral_link,$product['url_main'].'?link=')){$type='link';$link=str_replace($product['url_main'].'?link=','',$referral_link);}

if($type==''){answer_json('referral_link',$lng['error_field_required']);}

if($type=='user' && (!is_numeric($link) || $link<=0)){answer_json('referral_link',$lng['error_field_required']);}
if($type=='link' && strlen($link)!=8){answer_json('referral_link',$lng['error_field_required']);}

if($type=='user'){
$query=$db->prepare("SELECT * FROM users WHERE id=? AND isactive=? AND product_id=?");
$query->execute(array($link,'1',$product['id']));
if($query->rowCount()==0){answer_json('referral_link',$lng['error_referral_link']);}
$usr_row=$query->fetch(\PDO::FETCH_ASSOC);
...
if($usr_row['email']==''){$usr_row['email']=$usr_row['oauth_email'];}
...
$link='';
for($i=0;$i<8;$i++){$link.=$chars{mt_rand(0,strlen($chars)-1)};}

$query=$db->prepare("SELECT * FROM referral_links WHERE link=? AND product_id=?");
$query->execute(array($link,$product['id']));
if($query->rowCount()>0){answer_json('comment',$lng['error_system']);}
...
if(!$query->execute(array_values(array($balance_new,$user_id,$product['id'])))){answer_json('error',$lng['error_system']);}else{answer_json('success',$lng['admin_tokens_succcess'].' <b>'.$balance_new.'</b>');}
