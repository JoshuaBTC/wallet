...
$signup_oauth=$auth->signup_OAuth($oauth_id,$_GET['oauth'],array_merge($special_fields,array('balance_bonus'=>$product['signup_tokens'],'ip'=>$user['ip'],'country_code'=>$user['country_code'],'referrer'=>$_SESSION['ref'],'referral_link'=>$_SESSION['link'])));
...
$query = $db->prepare("UPDATE users SET referrals = referrals + ? WHERE id = ? AND product_id = ?");
$query->execute(array('1',$_SESSION['ref'],$product['id']));
}

if($_SESSION['link']!=NULL){
$query = $db->prepare("UPDATE referral_links SET referrals = referrals + ? WHERE link = ? AND product_id = ?");
...
$signup=$auth->signup($email,$password,$password_confirmation,array('balance_bonus'=>$product['signup_tokens'],'ip'=>$user['ip'],'country_code'=>$user['country_code'],'ethereum_address_provided'=>$ethereum_provided,'p'=>base64_encode($password),'referrer'=>$_SESSION['ref'],'referral_link'=>$_SESSION['link']),$product['email_activation'],$recaptcha);
